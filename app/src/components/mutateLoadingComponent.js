...
export default (options = { name: 'mutation' }) => SourceComponent => {
	class MutationState extends Component {
			...
			set = (param = { error: this.state.error }) => {
				...
				if (typeof loading !== 'boolean') {
					throw new Error(
						`You must provide 'loading' in ${options.name}.set function. For example: ${options.name}.set({ loading: true })`
					);
				}
	...
	MutationState.displayName = `MutateState(${getDisplayName(
		SourceComponent
	)})`;
